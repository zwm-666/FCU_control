...
export const Gauge: React.FC<GaugeProps> = ({ label, value, unit, color = "text-blue-400", min = 0, max = 100, size = 'normal' }) => {
  ...
  if (size === 'small') {
    return (
      <div className="bg-slate-50 p-2 rounded border border-slate-300 shadow-sm flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <div className="text-slate-400 text-[10px]">{unit}</div>
          </div>
        </div>
  ...
  return (
    <div className="bg-slate-50 p-3 rounded-lg border border-slate-300 shadow-sm">
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="text-slate-400 text-sm mb-1 ml-1">{unit}</div>
      </div>
      {/* Mini Progress Bar */}
