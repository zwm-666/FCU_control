"""
H2 FCU transport to WebSocket bridge server.
Reads FCU data from RS485, CAN, or simulation and forwards it to the frontend via WebSocket.
"""
...
from config import (
    ...
    DIAGNOSIS_ENABLE_MODEL_PREDICTION,
...
class ModbusRTUDriver(CANDriver):
    """Modbus RTU master transport over USB-RS485."""
    ...
    def __init__(
        ...
        self.parity = normalize_modbus_parity(parity)
    ...
    def _log_serial_frame(self, direction: str, register: int, frame: bytes):
        logger.info(
            f"Modbus RTU {direction} reg=0x{register:04X} len={len(frame)} data={self._format_hex(frame)}"
        )

    def _drain_stale_input(self, register: int):
        pending = getattr(self.serial, "in_waiting", 0)
        if not pending:
            return

        stale = self.serial.read(pending)
        logger.warning(
            f"Discarding stale Modbus RTU RX bytes before new request at register 0x{register:04X}: "
            f"len={len(stale)} data={self._format_hex(stale)}"
        )
    ...
    def open(self) -> bool:
        try:
            ...
            logger.info(
                f"Modbus RTU opened: port={self.port}, baudrate={self.baudrate}, parity={self.parity}, slave={self.slave_id}"
            )
            return True
        except ImportError:
            logger.error(
                "pyserial is not installed. "
                "If you are using Modbus RTU, run `cd backend && pip install -r requirements.txt` "
                "or `pip install -r backend/requirements.txt` from the repo root. "
                'If you do not have RS485 hardware, set CAN_INTERFACE_TYPE = "virtual" in backend/config.py.'
            )
        except Exception as e:
            logger.error(f"Failed to open Modbus RTU serial port {self.port}: {e}")
        return False

    def init_can(self, bitrate: int) -> bool:
        logger.info("Modbus RTU transport does not use CAN bitrate; serial baudrate is configured separately")
        return self.serial is not None

    def start(self) -> bool:
        self.running = self.serial is not None and self.serial.is_open
        if self.running:
            logger.info("Modbus RTU transport started")
        return self.running
    ...
    def close(self):
        ...
        if self.serial and self.serial.is_open:
            self.serial.close()
            logger.info("Modbus RTU serial closed")
    ...
    def receive(self, batch_size=100) -> List[Dict]:
        ...
        block = STATUS_BLOCKS[self._poll_index]
        self._poll_index = (self._poll_index + 1) % len(STATUS_BLOCKS)
        ...
        expected_len = 3 + block.quantity * 2 + 2

        try:
            self._drain_stale_input(block.start_register)
            self.serial.reset_input_buffer()
            self._log_serial_frame("TX", block.start_register, request)
            self.serial.write(request)
            self.serial.flush()
            response = self.serial.read(expected_len)
            self._log_serial_frame("RX", block.start_register, response)
            if len(response) != expected_len:
                logger.warning(
                    f"Modbus RTU RX length mismatch at register 0x{block.start_register:04X}: "
                    f"expected={expected_len}, actual={len(response)}"
                )
                return []

            payload = decode_read_holding_registers_response(
                response,
                self.slave_id,
                block.quantity,
            )
            return [
                {
                    "arbitration_id": block.can_id,
                    "data": payload,
                    "is_extended_id": True,
                    "timestamp": int(time.time() * 1000),
                    "modbus_register": block.start_register,
                    "modbus_type": block.message_type,
                }
            ][:batch_size]
        except Exception as e:
            logger.warning(f"Modbus RTU poll failed at register 0x{block.start_register:04X}: {e}")
            return []
    ...
    def send(self, arbitration_id, data, is_extended=True) -> bool:
        ...
        if arbitration_id != CAN_TX_ID:
            logger.warning(f"No Modbus write mapping for CAN ID 0x{arbitration_id:08X}")
            return False
        ...
        try:
            ...
            self._drain_stale_input(CONTROL_REGISTER)
            self.serial.reset_input_buffer()
            self._log_serial_frame("TX", CONTROL_REGISTER, request)
            self.serial.write(request)
            self.serial.flush()
            response = self.serial.read(8)
            self._log_serial_frame("RX", CONTROL_REGISTER, response)
            ...
            logger.warning(f"Modbus RTU control write failed: {e}")
            return False


class RS485Driver(CANDriver):
    """Custom RS485 transport over USB-RS485."""
    ...
    def _log_serial_frame(self, direction: str, frame: bytes):
        logger.info(f"RS485 {direction} len={len(frame)} data={self._format_hex(frame)}")
    ...
    def open(self) -> bool:
        try:
            ...
            logger.info(
                f"RS485 opened: port={self.port}, baudrate={self.baudrate}, timeout={self.timeout}"
            )
            return True
        except ImportError:
            logger.error(
                "pyserial is not installed. "
                "If you are using RS485, run `cd backend && pip install -r requirements.txt` "
                "or `pip install -r backend/requirements.txt` from the repo root. "
                'If you do not have RS485 hardware, set CAN_INTERFACE_TYPE = "virtual" in backend/config.py.'
            )
        except Exception as e:
            logger.error(f"Failed to open RS485 serial port {self.port}: {e}")
        return False

    def init_can(self, bitrate: int) -> bool:
        logger.info("RS485 transport does not use CAN bitrate; serial baudrate is configured separately")
        return self.serial is not None

    def start(self) -> bool:
        self.running = self.serial is not None and self.serial.is_open
        if self.running:
            logger.info("RS485 transport started")
        return self.running
    ...
    def close(self):
        ...
        if self.serial and self.serial.is_open:
            self.serial.close()
            logger.info("RS485 serial closed")
    ...
    def receive(self, batch_size=100) -> List[Dict]:
        ...
        try:
            ...
            self._log_serial_frame("RX", chunk)
            messages = self.parser.feed(chunk)
            if chunk and not messages and len(self.parser._buffer) == 0:
                logger.warning(
                    f"Discarding invalid RS485 bytes: len={len(chunk)} data={self._format_hex(chunk)}"
                )
            ...
            logger.warning(f"RS485 receive failed: {e}")
            return []
    ...
    def send(self, arbitration_id, data, is_extended=True) -> bool:
        ...
        payload = bytes(data)
        try:
            frame = build_rs485_frame(0x10, payload)
            self._log_serial_frame("TX", frame)
            self.serial.write(frame)
            self.serial.flush()
            return True
        except Exception as e:
            logger.warning(f"RS485 control write failed: {e}")
            return False
...
class CANWebSocketServer:
    """FCU transport to WebSocket bridge server"""
    ...
    def __init__(self):
        ...
        if DIAGNOSIS_AVAILABLE:
            try:
                ...
                self.diagnosis = OnlineDiagnosis(
                    model_dir=model_dir,
                    enable_model_prediction=DIAGNOSIS_ENABLE_MODEL_PREDICTION,
                )
    ...
    async def start(self):
        ...
        try:
            ...
            if not self.driver.open():
                logger.error("Failed to open transport device. Exiting.")
                return

            if not self.driver.init_can(CAN_BITRATE):
                logger.error("Failed to initialize transport. Exiting.")
                self.driver.close()
                return
                
            if not self.driver.start():
                logger.error("Failed to start transport communication. Exiting.")
                self.driver.close()
                return
                
            logger.info(f"✓ FCU transport initialized ({CAN_INTERFACE_TYPE} Driver)")
    ...
    async def can_receive_loop(self):
        """Continuously read FCU transport messages"""
        logger.info("FCU receive loop started")
            ...
            try:
                ...
                if messages:
                    ...
                    for msg_dict in messages:
                        ...
                        logger.debug(f"📥 FCU RX | ID: 0x{msg_dict['arbitration_id']:08X} | 数据: {data_formatted}")
                ...
                if current_time - last_stats_time >= 30:
                    logger.info(f"📊 FCU接收统计: 轮询次数={poll_count}, 收到消息={msg_count}")
                    if msg_count == 0:
                        logger.warning("⚠️ 警告: 30秒内未收到任何FCU消息，请检查串口/FCU是否正常发送数据")
                ...
                logger.info("FCU receive loop cancelled")
                break
            except Exception as e:
                logger.error(f"Error in FCU receive loop: {e}")
                await asyncio.sleep(0.1)
                
    async def process_can_message(self, msg):
        """Process incoming FCU message"""
        ...
        if can_id in MESSAGE_PARSERS:
            try:
                ...
                if can_id == 0x18FF01F0 and self.machine_state.status.get("heartbeat", 0) % 100 == 0:
                    logger.info(f"FCU RX: 0x{can_id:08X} - Heartbeat: {self.machine_state.status.get('heartbeat')}")
            except Exception as e:
            ...
            if can_id not in self._unknown_ids:
                ...
                logger.warning(f"⚠️ 收到未识别的FCU消息ID: 0x{can_id:08X}, 数据: {msg.data.hex()}")
    ...
    async def handle_client_message(self, message: str):
        ...
        try:
            ...
            if msg_type == "control":
                ...
                # Generate transport packets. The payload stays compatible with the original CAN protocol.
                packets = generate_control_packets(control)
                
                # Send all packets to the configured FCU transport.
                sent_count = 0
                for can_id, can_data in packets:
                    success = self.driver.send(can_id, can_data, is_extended=True)
                    if success:
                        sent_count += 1
                        logger.info(f"📤 FCU TX | ID: 0x{can_id:08X} | 数据: {can_data.hex().upper()}")
                    else:
                        logger.warning(f"❌ Failed to send FCU TX: 0x{can_id:08X}")
    ...
    def stop(self):
        ...
        if self.driver:
            self.driver.close()
            logger.info("FCU transport closed")


async def main():
    """Main entry point"""
    logger.info("=" * 60)
    logger.info("H2 FCU Transport to WebSocket Bridge Server")
    logger.info("=" * 60)
