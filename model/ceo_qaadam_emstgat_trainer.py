...
def load_and_preprocess_data(csv_path: str, test_size: float = 0.2, seed: int = 42):
    ...
    logger.info(f"选择 {len(selected_idx)}/{len(feature_names)} 个特征（累计重要性≥95%）")
...
def run_training_pipeline(csv_path: str, test_size: float = 0.2, epochs: int = 100,
    ...
    plot_confusion_matrix(y_test, y_pred, class_names, os.path.join(output_dir, 'confusion_matrix.png'))
...
if __name__ == "__main__":
    ...
    project_root = os.path.dirname(os.path.dirname(script_dir))  # pythonProject3
